...
| **`/develop-frontier`** | Sync and land the latest skeleton before every wave; then run one subagent per parallel-capable ticket (**TEST→DEV→VAL** per child worktree under **`.worktrees/FR-NNNN-<slug>/`**), merge to **`feat/…`**, and use **`finish-feature`** only after the **§2d** gate. |
